...
CREATE OR REPLACE FUNCTION "public"."check_login_lock"("p_email" "text") RETURNS TABLE("locked" boolean, "locked_until" timestamp with time zone, "attempts_remaining" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  return query select false, null::timestamptz, greatest(0, v_max - v_row.attempt_count);
end;
$$;


ALTER FUNCTION "public"."check_login_lock"("p_email" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."clear_login_lock"("p_email" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
...
ALTER FUNCTION "public"."clear_login_lock"("p_email" "text") OWNER TO "postgres";
...
begin
  ...
  exception when others then
    raise warning 'create_wallet_for_new_user failed for user %: %', new.id, sqlerrm;
  end;
  return new;
end;
$$;
...
CREATE OR REPLACE FUNCTION "public"."expire_channel_subscription"("p_user_id" "uuid", "p_channel_id" "uuid") RETURNS "void"
...
ALTER FUNCTION "public"."expire_channel_subscription"("p_user_id" "uuid", "p_channel_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."notify_new_group_message_push"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  loop
    perform net.http_post(
      url := 'https://azmbhcyoubsnvxiyvdro.supabase.co/functions/v1/send-fcm-push',
      headers := jsonb_build_object('Content-Type', 'application/json', 'x-push-secret', 'rrXkrAyKJqFVtXbXb6h-ZzkPfCFJLklaJGG6b8A2h4g'),
      body := jsonb_build_object(
        'user_id', member_id,
        'title', coalesce(group_name, 'New group message'),
        'body', coalesce(sender_name, 'Someone') || ' sent a message',
        'url', '/Dgreatverse/?group=' || new.group_id
      ),
...
ALTER FUNCTION "public"."notify_new_group_message_push"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."notify_new_private_message_push"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  perform net.http_post(
    ...
    body := jsonb_build_object(
      ...
      'url', '/Dgreatverse/?chat=' || new.sender_id
    ),
    timeout_milliseconds := 5000
  );
  return new;
...
ALTER FUNCTION "public"."notify_new_private_message_push"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."protect_channel_protected_columns"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
begin
  ...
  if new.is_verified is distinct from old.is_verified
     or new.maintenance_paid_until is distinct from old.maintenance_paid_until
     or new.is_snoozed is distinct from old.is_snoozed
  then
    raise exception 'protected channel columns can only be changed by a trusted backend function';
  end if;
...
ALTER FUNCTION "public"."protect_channel_protected_columns"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."record_login_attempt"("p_email" "text", "p_success" boolean) RETURNS TABLE("locked" boolean, "locked_until" timestamp with time zone, "attempts_remaining" integer)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  v_row login_attempts%rowtype;
  v_max int := 5;
  v_lock_duration interval := interval '1 hour';
  v_new_count int;
...
begin
  ...
  if not found or (v_row.locked_until is not null and v_row.locked_until <= now()) then
    v_new_count := 1;
  else
  ...
  if v_new_count >= v_max then
    v_new_locked_until := now() + v_lock_duration;
  else
    v_new_locked_until := null;
  end if;

  insert into login_attempts (email, attempt_count, locked_until, last_attempt_at)
  values (v_email, v_new_count, v_new_locked_until, now())
  on conflict (email) do update
    set attempt_count = excluded.attempt_count,
        locked_until = excluded.locked_until,
        last_attempt_at = excluded.last_attempt_at;

  if v_new_locked_until is not null then
    return query select true, v_new_locked_until, 0;
  else
    return query select false, null::timestamptz, greatest(0, v_max - v_new_count);
  end if;
end;
$$;


ALTER FUNCTION "public"."record_login_attempt"("p_email" "text", "p_success" boolean) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text") RETURNS TABLE("amount_charged_usd" numeric, "expires_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  v_price numeric;
  v_price_usd numeric;
  v_ngn_per_usd numeric := 1500; -- must match NGN_PER_USD in index.html
  v_subscriber_balance numeric;
  v_existing_expiry timestamptz;
  v_base timestamptz;
  v_expires timestamptz;
begin
  if p_plan not in ('monthly', 'yearly') then
    raise exception 'plan must be monthly or yearly';
  end if;

  select owner_id, has_subscription, sub_currency, sub_price_monthly_ngn, sub_price_yearly_ngn
    into v_owner_id, v_has_subscription, v_currency, v_price_monthly, v_price_yearly
    from channels where id = p_channel_id for update;

  if v_owner_id is null then raise exception 'channel not found'; end if;
  if not v_has_subscription then raise exception 'this channel does not offer subscriptions'; end if;
  if v_owner_id = p_user_id then raise exception 'you own this channel — no need to subscribe to it'; end if;

  v_price := case when p_plan = 'yearly' then v_price_yearly else v_price_monthly end;
  if v_price is null or v_price <= 0 then
    raise exception 'this channel has no subscription price configured';
  end if;

  v_price_usd := case when v_currency = 'USD' then v_price else round(v_price / v_ngn_per_usd, 2) end;

  select cash_balance_usd into v_subscriber_balance from users where id = p_user_id for update;
  if v_subscriber_balance is null or v_subscriber_balance < v_price_usd then
    raise exception 'insufficient wallet balance';
  end if;

  update users set cash_balance_usd = cash_balance_usd - v_price_usd where id = p_user_id;
  update users set cash_balance_usd = coalesce(cash_balance_usd, 0) + v_price_usd where id = v_owner_id;

  select cs.expires_at into v_existing_expiry
    from channel_subscriptions cs where cs.channel_id = p_channel_id and cs.user_id = p_user_id;

  v_base := case when v_existing_expiry is not null and v_existing_expiry > now() then v_existing_expiry else now() end;
  v_expires := case when p_plan = 'yearly' then v_base + interval '1 year' else v_base + interval '1 month' end;

  insert into channel_subscriptions (channel_id, user_id, plan, price_ngn, currency, amount_charged_usd, started_at, expires_at, status)
  values (p_channel_id, p_user_id, p_plan, v_price, v_currency, v_price_usd, now(), v_expires, 'active')
  on conflict (channel_id, user_id)
  do update set plan = excluded.plan, price_ngn = excluded.price_ngn, currency = excluded.currency,
                amount_charged_usd = excluded.amount_charged_usd, expires_at = excluded.expires_at, status = 'active';

  return query select v_price_usd, v_expires;
end;
$$;


ALTER FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text", "p_auto_renew" boolean DEFAULT NULL::boolean) RETURNS TABLE("amount_charged_usd" numeric, "expires_at" timestamp with time zone)
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  v_price numeric;
  v_price_usd numeric;
  v_ngn_per_usd numeric := 1500; -- must match NGN_PER_USD in index.html / both handlers
  v_subscriber_balance numeric;
  v_existing_expiry timestamptz;
  v_base timestamptz;
  v_expires timestamptz;
begin
  ...
  insert into channel_subscriptions (channel_id, user_id, plan, currency, amount_charged_usd, expires_at, status, auto_renew)
  values (p_channel_id, p_user_id, p_plan, v_currency, v_price_usd, v_expires, 'active', coalesce(p_auto_renew, true))
  on conflict (channel_id, user_id)
  do update set plan = excluded.plan, currency = excluded.currency,
                amount_charged_usd = excluded.amount_charged_usd, expires_at = excluded.expires_at,
                status = 'active',
                auto_renew = coalesce(p_auto_renew, channel_subscriptions.auto_renew);
...
ALTER FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text", "p_auto_renew" boolean) OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."cash_tips" (
    ...
    "group_id" "uuid",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "cash_tips_amount_usd_check" CHECK (("amount_usd" > (0)::numeric))
);


ALTER TABLE "public"."cash_tips" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."channel_follow_payments" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "channel_id" "uuid" NOT NULL,
    "follower_id" "uuid" NOT NULL,
    "owner_id" "uuid" NOT NULL,
    "amount_usd" numeric(10,2) NOT NULL,
    "price_dg_at_purchase" integer,
    "provider" "text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "price_ngn_at_purchase" numeric,
    CONSTRAINT "channel_follow_payments_amount_usd_check" CHECK (("amount_usd" > (0)::numeric)),
    CONSTRAINT "channel_follow_payments_price_dg_at_purchase_check" CHECK (("price_dg_at_purchase" > 0)),
    CONSTRAINT "channel_follow_payments_price_ngn_at_purchase_check" CHECK ((("price_ngn_at_purchase" IS NULL) OR ("price_ngn_at_purchase" > (0)::numeric))),
    CONSTRAINT "channel_follow_payments_provider_check" CHECK (("provider" = ANY (ARRAY['stripe'::"text", 'paystack'::"text"])))
);


ALTER TABLE "public"."channel_follow_payments" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."channel_subscriptions" (
    ...
    CONSTRAINT "channel_subscriptions_plan_check" CHECK (("plan" = ANY (ARRAY['monthly'::"text", 'yearly'::"text"]))),
...
CREATE TABLE IF NOT EXISTS "public"."channels" (
    ...
    "sub_price_monthly_ngn" numeric(10,2),
    "sub_price_yearly_ngn" numeric(10,2),
    "sub_currency" "text" DEFAULT 'NGN'::"text" NOT NULL,
    "is_ad_master" boolean DEFAULT false NOT NULL,
    "is_verified" boolean DEFAULT false NOT NULL,
    "maintenance_paid_until" timestamp with time zone,
    ...
    CONSTRAINT "channels_private_paid_exclusive" CHECK ((NOT ("is_private" AND "is_paid"))),
    CONSTRAINT "channels_sub_monthly_cap" CHECK ((("sub_price_monthly_ngn" IS NULL) OR (("sub_price_monthly_ngn" > (0)::numeric) AND ("sub_price_monthly_ngn" <= (7000)::numeric)))),
    CONSTRAINT "channels_sub_yearly_positive" CHECK ((("sub_price_yearly_ngn" IS NULL) OR ("sub_price_yearly_ngn" > (0)::numeric)))
);
...
CREATE OR REPLACE VIEW "public"."channel_posts_secure" WITH ("security_invoker"='false') AS
 SELECT "cp"."id",
    "cp"."channel_id",
    "cp"."caption",
    "cp"."media_type",
    ...
    "cp"."price_ngn",
        ...
        CASE
            WHEN "perm"."mask" THEN NULL::"text"
            ELSE "cp"."media_public_id"
        END AS "media_public_id",
    (("cp"."media_url" IS NOT NULL) OR ("cp"."media_urls" IS NOT NULL)) AS "has_media"
   FROM ((("public"."channel_posts" "cp"
     JOIN "public"."channels" "c" ON (("c"."id" = "cp"."channel_id")))
     CROSS JOIN LATERAL ( SELECT ((COALESCE("cp"."price_ngn", (0)::numeric) > (0)::numeric) OR (COALESCE("cp"."price_usd", (0)::numeric) > (0)::numeric)) AS "is_paid",
            (("auth"."uid"() IS NOT NULL) AND ("c"."owner_id" = "auth"."uid"())) AS "is_owner",
            (("auth"."uid"() IS NOT NULL) AND (EXISTS ( SELECT 1
                   FROM "public"."paid_unlocks" "pu"
                  WHERE (("pu"."content_type" = 'channel_post'::"text") AND ("pu"."content_id" = ("cp"."id")::"text") AND ("pu"."user_id" = "auth"."uid"()))))) AS "is_unlocked_row") "base")
     CROSS JOIN LATERAL ( SELECT ("base"."is_paid" AND (NOT "base"."is_owner") AND (NOT "base"."is_unlocked_row")) AS "mask") "perm")
  WHERE ((NOT ("c"."is_gated" OR "c"."has_subscription")) OR (("auth"."uid"() IS NOT NULL) AND ("c"."owner_id" = "auth"."uid"())) OR (("auth"."uid"() IS NOT NULL) AND (EXISTS ( SELECT 1
           FROM "public"."channel_subscriptions" "s"
          WHERE (("s"."channel_id" = "cp"."channel_id") AND ("s"."user_id" = "auth"."uid"()) AND ("s"."status" = 'active'::"text") AND ("s"."expires_at" > "now"()))))));


ALTER VIEW "public"."channel_posts_secure" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."channel_subscription_payments" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "channel_id" "uuid" NOT NULL,
    ...
    "plan" "text" NOT NULL,
    "amount_usd" numeric(10,2) NOT NULL,
    "price_ngn_at_purchase" numeric(10,2),
    "provider" "text" NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "channel_subscription_payments_plan_check" CHECK (("plan" = ANY (ARRAY['monthly'::"text", 'yearly'::"text"]))),
    CONSTRAINT "channel_subscription_payments_provider_check" CHECK (("provider" = ANY (ARRAY['stripe'::"text", 'paystack'::"text"])))
);


ALTER TABLE "public"."channel_subscription_payments" OWNER TO "postgres";
...
CREATE OR REPLACE VIEW "public"."group_messages_secure" WITH ("security_invoker"='false') AS
 SELECT "gm"."id",
    "gm"."group_id",
    "gm"."sender_id",
    "gm"."content",
    ...
    "gm"."price_ngn",
        ...
        CASE
            ...
            ELSE "gm"."public_id"
        END AS "public_id",
    (("gm"."file_url" IS NOT NULL) OR ("gm"."file_urls" IS NOT NULL)) AS "has_media",
    "gm"."preview_media_url",
    "gm"."preview_media_public_id"
   FROM (("public"."group_messages" "gm"
     CROSS JOIN LATERAL ( SELECT ((COALESCE("gm"."price_usd", (0)::numeric) > (0)::numeric) OR (COALESCE("gm"."price_ngn", (0)::numeric) > (0)::numeric)) AS "is_paid",
            (("auth"."uid"() IS NOT NULL) AND ("gm"."sender_id" = "auth"."uid"())) AS "is_sender",
            (("auth"."uid"() IS NOT NULL) AND (EXISTS ( SELECT 1
                   FROM "public"."paid_unlocks" "pu"
                  WHERE (("pu"."content_type" = 'group_message'::"text") AND ("pu"."content_id" = ("gm"."id")::"text") AND ("pu"."user_id" = "auth"."uid"()))))) AS "is_unlocked") "base")
     CROSS JOIN LATERAL ( SELECT ("base"."is_paid" AND (NOT "base"."is_sender") AND (NOT "base"."is_unlocked")) AS "mask") "perm")
  WHERE ((("auth"."uid"() IS NOT NULL) AND "public"."is_group_member"("gm"."group_id", "auth"."uid"())) OR (("auth"."uid"() IS NOT NULL) AND ("gm"."sender_id" = "auth"."uid"())));


ALTER VIEW "public"."group_messages_secure" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."cash_tips"
    ADD CONSTRAINT "cash_tips_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."channel_follow_payments"
    ADD CONSTRAINT "channel_follow_payments_channel_id_follower_id_key" UNIQUE ("channel_id", "follower_id");



ALTER TABLE ONLY "public"."channel_follow_payments"
    ADD CONSTRAINT "channel_follow_payments_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."channel_subscription_payments"
    ADD CONSTRAINT "channel_subscription_payments_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."channel_subscriptions"
    ADD CONSTRAINT "channel_subscriptions_channel_user_unique" UNIQUE ("channel_id", "user_id");
...
ALTER TABLE ONLY "public"."wallet_topups"
    ADD CONSTRAINT "wallet_topups_reference_key" UNIQUE ("reference");
...
CREATE INDEX "idx_cash_tips_receiver_created" ON "public"."cash_tips" USING "btree" ("receiver_id", "created_at" DESC);
...
CREATE OR REPLACE TRIGGER "trg_notify_group_message_push" AFTER INSERT ON "public"."group_messages" FOR EACH ROW EXECUTE FUNCTION "public"."notify_new_group_message_push"();



CREATE OR REPLACE TRIGGER "trg_notify_private_message_push" AFTER INSERT ON "public"."messages" FOR EACH ROW EXECUTE FUNCTION "public"."notify_new_private_message_push"();



CREATE OR REPLACE TRIGGER "trg_protect_channel_columns" BEFORE UPDATE ON "public"."channels" FOR EACH ROW EXECUTE FUNCTION "public"."protect_channel_protected_columns"();
...
ALTER TABLE ONLY "public"."cash_tips"
    ADD CONSTRAINT "cash_tips_group_id_fkey" FOREIGN KEY ("group_id") REFERENCES "public"."groups"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."cash_tips"
    ADD CONSTRAINT "cash_tips_receiver_id_fkey" FOREIGN KEY ("receiver_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."cash_tips"
    ADD CONSTRAINT "cash_tips_sender_id_fkey" FOREIGN KEY ("sender_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."channel_follow_payments"
    ADD CONSTRAINT "channel_follow_payments_channel_id_fkey" FOREIGN KEY ("channel_id") REFERENCES "public"."channels"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."channel_follow_payments"
    ADD CONSTRAINT "channel_follow_payments_follower_id_fkey" FOREIGN KEY ("follower_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."channel_follow_payments"
    ADD CONSTRAINT "channel_follow_payments_owner_id_fkey" FOREIGN KEY ("owner_id") REFERENCES "public"."users"("id");
...
ALTER TABLE ONLY "public"."channel_subscription_payments"
    ADD CONSTRAINT "channel_subscription_payments_channel_id_fkey" FOREIGN KEY ("channel_id") REFERENCES "public"."channels"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."channel_subscription_payments"
    ADD CONSTRAINT "channel_subscription_payments_owner_id_fkey" FOREIGN KEY ("owner_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."channel_subscription_payments"
    ADD CONSTRAINT "channel_subscription_payments_subscriber_id_fkey" FOREIGN KEY ("subscriber_id") REFERENCES "public"."users"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."wallet_topups"
    ADD CONSTRAINT "wallet_topups_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."users"("id");
...
CREATE POLICY "Group members can read messages" ON "public"."group_messages" FOR SELECT USING ("public"."is_group_member"("group_id", "auth"."uid"()));
...
CREATE POLICY "Users can insert tips they send" ON "public"."cash_tips" FOR INSERT WITH CHECK (("auth"."uid"() = "sender_id"));
...
CREATE POLICY "Users can view own topups" ON "public"."topups" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can view own wallet topups" ON "public"."wallet_topups" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can view their own tips" ON "public"."cash_tips" FOR SELECT USING ((("auth"."uid"() = "sender_id") OR ("auth"."uid"() = "receiver_id")));
...
ALTER TABLE "public"."channel_follow_payments" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "channel_follow_payments_select_own" ON "public"."channel_follow_payments" FOR SELECT USING ((("follower_id" = "auth"."uid"()) OR ("owner_id" = "auth"."uid"())));
...
CREATE POLICY "channel_follows_insert_guarded" ON "public"."channel_follows" FOR INSERT WITH CHECK (((("user_id" = "auth"."uid"()) AND (EXISTS ( SELECT 1
   FROM "public"."channels" "c"
  WHERE (("c"."id" = "channel_follows"."channel_id") AND ("c"."is_paid" IS NOT TRUE))))) OR (EXISTS ( SELECT 1
   FROM ("public"."channel_join_requests" "r"
     JOIN "public"."channels" "c" ON (("c"."id" = "r"."channel_id")))
  WHERE (("r"."channel_id" = "channel_follows"."channel_id") AND ("r"."from_user" = "channel_follows"."user_id") AND ("r"."to_user" = "auth"."uid"()) AND ("r"."status" = 'accepted'::"text") AND ("c"."is_paid" IS NOT TRUE))))));
...
CREATE POLICY "channel_posts_select_gated" ON "public"."channel_posts" FOR SELECT USING (((NOT (EXISTS ( SELECT 1
   FROM "public"."channels" "c"
  WHERE (("c"."id" = "channel_posts"."channel_id") AND ("c"."is_gated" OR "c"."has_subscription"))))) OR (EXISTS ( SELECT 1
   FROM "public"."channels" "c"
  WHERE (("c"."id" = "channel_posts"."channel_id") AND ("c"."owner_id" = "auth"."uid"())))) OR (EXISTS ( SELECT 1
   FROM "public"."channel_subscriptions" "s"
  WHERE (("s"."channel_id" = "channel_posts"."channel_id") AND ("s"."user_id" = "auth"."uid"()) AND ("s"."status" = 'active'::"text") AND ("s"."expires_at" > "now"()))))));
...
CREATE POLICY "channel_sub_payments_select_own_or_owner" ON "public"."channel_subscription_payments" FOR SELECT USING ((("subscriber_id" = "auth"."uid"()) OR ("owner_id" = "auth"."uid"())));



ALTER TABLE "public"."channel_subscription_payments" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "fcm_tokens_delete_own" ON "public"."fcm_tokens" FOR DELETE USING (("user_id" = "auth"."uid"()));



CREATE POLICY "fcm_tokens_insert_own" ON "public"."fcm_tokens" FOR INSERT WITH CHECK (("user_id" = "auth"."uid"()));



CREATE POLICY "fcm_tokens_select_own" ON "public"."fcm_tokens" FOR SELECT USING (("user_id" = "auth"."uid"()));



CREATE POLICY "fcm_tokens_update_own" ON "public"."fcm_tokens" FOR UPDATE USING (("user_id" = "auth"."uid"()));
...
CREATE POLICY "only channel owner can post" ON "public"."channel_posts" FOR INSERT WITH CHECK (("channel_id" IN ( SELECT "channels"."id"
   FROM "public"."channels"
  WHERE ("channels"."owner_id" = "auth"."uid"()))));
...
CREATE POLICY "paid_unlocks_select_own" ON "public"."paid_unlocks" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "select_own_channel_subscription" ON "public"."channel_subscriptions" FOR SELECT TO "authenticated" USING (("user_id" = "auth"."uid"()));
...
CREATE POLICY "users can only post own stories" ON "public"."stories" FOR INSERT WITH CHECK (("user_id" = "auth"."uid"()));
...
REVOKE ALL ON FUNCTION "public"."activate_vip"("p_user_id" "uuid", "p_plan" "text") FROM PUBLIC;
...
GRANT ALL ON FUNCTION "public"."check_login_lock"("p_email" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."check_login_lock"("p_email" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_login_lock"("p_email" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."clear_login_lock"("p_email" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."clear_login_lock"("p_email" "text") TO "service_role";



REVOKE ALL ON FUNCTION "public"."complete_topup"("p_payment_reference" "text") FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."expire_channel_subscription"("p_user_id" "uuid", "p_channel_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."expire_channel_subscription"("p_user_id" "uuid", "p_channel_id" "uuid") TO "service_role";



REVOKE ALL ON FUNCTION "public"."handle_channel_follow_count"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."handle_channel_follow_count"() TO "service_role";



REVOKE ALL ON FUNCTION "public"."handle_channel_post_comment_count"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."handle_channel_post_comment_count"() TO "service_role";



REVOKE ALL ON FUNCTION "public"."handle_channel_post_like_count"() FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."next_ad_indices"("p_count" integer, "p_total" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."next_ad_indices"("p_count" integer, "p_total" integer) TO "service_role";



REVOKE ALL ON FUNCTION "public"."next_zone_ad_indices"("p_count" integer, "p_total" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."next_zone_ad_indices"("p_count" integer, "p_total" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."notify_new_group_message_push"() TO "anon";
GRANT ALL ON FUNCTION "public"."notify_new_group_message_push"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."notify_new_group_message_push"() TO "service_role";



GRANT ALL ON FUNCTION "public"."notify_new_private_message_push"() TO "anon";
GRANT ALL ON FUNCTION "public"."notify_new_private_message_push"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."notify_new_private_message_push"() TO "service_role";



REVOKE ALL ON FUNCTION "public"."protect_channel_protected_columns"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."protect_channel_protected_columns"() TO "service_role";



REVOKE ALL ON FUNCTION "public"."record_login_attempt"("p_email" "text", "p_success" boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."record_login_attempt"("p_email" "text", "p_success" boolean) TO "service_role";



REVOKE ALL ON FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text", "p_auto_renew" boolean) TO "authenticated";
GRANT ALL ON FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text", "p_auto_renew" boolean) TO "service_role";
...
GRANT ALL ON TABLE "public"."cash_tips" TO "anon";
GRANT ALL ON TABLE "public"."cash_tips" TO "authenticated";
GRANT ALL ON TABLE "public"."cash_tips" TO "service_role";



GRANT ALL ON TABLE "public"."channel_follow_payments" TO "anon";
GRANT ALL ON TABLE "public"."channel_follow_payments" TO "authenticated";
GRANT ALL ON TABLE "public"."channel_follow_payments" TO "service_role";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."channel_posts" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."channel_posts" TO "authenticated";
GRANT ALL ON TABLE "public"."channel_posts" TO "service_role";



GRANT SELECT("id") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("id") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("channel_id") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("channel_id") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("caption") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("caption") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("media_type") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("media_type") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("like_count") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("like_count") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("comment_count") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("comment_count") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("tip_total_dg") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("tip_total_dg") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("created_at") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("created_at") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("price_dg") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("price_dg") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("preview_text") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("preview_text") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("expires_at") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("expires_at") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("sponsor_deal_id") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("sponsor_deal_id") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("sponsor_name") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("sponsor_name") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("cta_url") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("cta_url") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("cta_label") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("cta_label") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("price_usd") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("price_usd") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("price_ngn") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("price_ngn") ON TABLE "public"."channel_posts" TO "authenticated";



GRANT SELECT("preview_media_url") ON TABLE "public"."channel_posts" TO "anon";
GRANT SELECT("preview_media_url") ON TABLE "public"."channel_posts" TO "authenticated";
...
GRANT SELECT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."channels" TO "anon";
GRANT SELECT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."channels" TO "authenticated";
GRANT ALL ON TABLE "public"."channels" TO "service_role";



GRANT INSERT("id"),UPDATE("id") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("id"),UPDATE("id") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("owner_id"),UPDATE("owner_id") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("owner_id"),UPDATE("owner_id") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("name"),UPDATE("name") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("name"),UPDATE("name") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("handle"),UPDATE("handle") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("handle"),UPDATE("handle") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("about"),UPDATE("about") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("about"),UPDATE("about") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("avatar_url"),UPDATE("avatar_url") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("avatar_url"),UPDATE("avatar_url") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("avatar_public_id"),UPDATE("avatar_public_id") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("avatar_public_id"),UPDATE("avatar_public_id") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("banner_url"),UPDATE("banner_url") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("banner_url"),UPDATE("banner_url") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("banner_public_id"),UPDATE("banner_public_id") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("banner_public_id"),UPDATE("banner_public_id") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("is_gated"),UPDATE("is_gated") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("is_gated"),UPDATE("is_gated") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("price_usd"),UPDATE("price_usd") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("price_usd"),UPDATE("price_usd") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("price_ngn"),UPDATE("price_ngn") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("price_ngn"),UPDATE("price_ngn") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("post_count"),UPDATE("post_count") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("post_count"),UPDATE("post_count") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("created_at"),UPDATE("created_at") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("created_at"),UPDATE("created_at") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("is_paid"),UPDATE("is_paid") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("is_paid"),UPDATE("is_paid") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("price_dg"),UPDATE("price_dg") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("price_dg"),UPDATE("price_dg") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("is_private"),UPDATE("is_private") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("is_private"),UPDATE("is_private") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("has_subscription"),UPDATE("has_subscription") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("has_subscription"),UPDATE("has_subscription") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("sub_price_monthly_ngn"),UPDATE("sub_price_monthly_ngn") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("sub_price_monthly_ngn"),UPDATE("sub_price_monthly_ngn") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("sub_price_yearly_ngn"),UPDATE("sub_price_yearly_ngn") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("sub_price_yearly_ngn"),UPDATE("sub_price_yearly_ngn") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("sub_currency"),UPDATE("sub_currency") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("sub_currency"),UPDATE("sub_currency") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("is_ad_master"),UPDATE("is_ad_master") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("is_ad_master"),UPDATE("is_ad_master") ON TABLE "public"."channels" TO "anon";



GRANT INSERT("is_verified"),UPDATE("is_verified") ON TABLE "public"."channels" TO "authenticated";
GRANT INSERT("is_verified"),UPDATE("is_verified") ON TABLE "public"."channels" TO "anon";
...
GRANT ALL ON TABLE "public"."channel_posts_secure" TO "anon";
GRANT ALL ON TABLE "public"."channel_posts_secure" TO "authenticated";
GRANT ALL ON TABLE "public"."channel_posts_secure" TO "service_role";



GRANT ALL ON TABLE "public"."channel_subscription_payments" TO "anon";
GRANT ALL ON TABLE "public"."channel_subscription_payments" TO "authenticated";
GRANT ALL ON TABLE "public"."channel_subscription_payments" TO "service_role";
...
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."group_messages" TO "anon";
GRANT INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN,UPDATE ON TABLE "public"."group_messages" TO "authenticated";
GRANT ALL ON TABLE "public"."group_messages" TO "service_role";



GRANT SELECT("id") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("group_id") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("sender_id") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("content") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("file_type") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("reply_to_id") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("created_at") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("expiry_warned") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("preview_text") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("price_usd") ON TABLE "public"."group_messages" TO "authenticated";



GRANT SELECT("price_ngn") ON TABLE "public"."group_messages" TO "authenticated";



GRANT ALL ON TABLE "public"."group_messages_secure" TO "anon";
GRANT ALL ON TABLE "public"."group_messages_secure" TO "authenticated";
GRANT ALL ON TABLE "public"."group_messages_secure" TO "service_role";
...
GRANT INSERT("id") ON TABLE "public"."users" TO "anon";



GRANT INSERT("name"),UPDATE("name") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("name"),UPDATE("name") ON TABLE "public"."users" TO "anon";



GRANT INSERT("email") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("email") ON TABLE "public"."users" TO "anon";



GRANT INSERT("chat_code"),UPDATE("chat_code") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("chat_code"),UPDATE("chat_code") ON TABLE "public"."users" TO "anon";



GRANT INSERT("avatar_url"),UPDATE("avatar_url") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("avatar_url"),UPDATE("avatar_url") ON TABLE "public"."users" TO "anon";



GRANT INSERT("about"),UPDATE("about") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("about"),UPDATE("about") ON TABLE "public"."users" TO "anon";



GRANT INSERT("created_at") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("created_at") ON TABLE "public"."users" TO "anon";



GRANT INSERT("avatar_public_id"),UPDATE("avatar_public_id") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("avatar_public_id"),UPDATE("avatar_public_id") ON TABLE "public"."users" TO "anon";



GRANT INSERT("is_vip"),UPDATE("is_vip") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("is_vip"),UPDATE("is_vip") ON TABLE "public"."users" TO "anon";



GRANT INSERT("vip_plan"),UPDATE("vip_plan") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("vip_plan"),UPDATE("vip_plan") ON TABLE "public"."users" TO "anon";



GRANT INSERT("vip_expires_at"),UPDATE("vip_expires_at") ON TABLE "public"."users" TO "authenticated";
GRANT INSERT("vip_expires_at"),UPDATE("vip_expires_at") ON TABLE "public"."users" TO "anon";
...
GRANT SELECT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."wallets" TO "anon";
GRANT SELECT,INSERT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."wallets" TO "authenticated";
GRANT ALL ON TABLE "public"."wallets" TO "service_role";



GRANT INSERT("user_id"),UPDATE("user_id") ON TABLE "public"."wallets" TO "authenticated";
GRANT INSERT("user_id"),UPDATE("user_id") ON TABLE "public"."wallets" TO "anon";



GRANT INSERT("updated_at"),UPDATE("updated_at") ON TABLE "public"."wallets" TO "authenticated";
GRANT INSERT("updated_at"),UPDATE("updated_at") ON TABLE "public"."wallets" TO "anon";
