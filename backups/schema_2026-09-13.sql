...
CREATE OR REPLACE VIEW "public"."channel_posts_secure" WITH ("security_invoker"='false') AS
 SELECT "cp"."id",
    ...
    "cp"."price_ngn",
        CASE
            WHEN ("perm"."mask" AND "perm"."first_is_doc") THEN NULL::"text"
            ELSE "cp"."media_url"
        END AS "media_url",
        CASE
            WHEN ("perm"."mask" AND ("cp"."media_urls" IS NOT NULL)) THEN ("perm"."masked_media_urls")::"text"
            ELSE "cp"."media_urls"
        END AS "media_urls",
        CASE
            WHEN ("perm"."mask" AND "perm"."first_is_doc") THEN NULL::"text"
   ...
   FROM (("public"."channel_posts" "cp"
     ...
     CROSS JOIN LATERAL ( SELECT "base"."is_paid",
            "base"."is_owner",
            "base"."is_unlocked_row",
            ("base"."is_paid" AND (NOT "base"."is_owner") AND (NOT "base"."is_unlocked_row")) AS "mask",
            (("cp"."media_type" IS NOT NULL) AND ("cp"."media_type" <> 'image'::"text") AND ("cp"."media_type" <> 'video'::"text")) AS "first_is_doc",
                CASE
                    WHEN ("cp"."media_urls" IS NOT NULL) THEN ( SELECT "jsonb_agg"(
                            CASE
                                WHEN (("arr"."ord" = 1) AND (("arr"."elem" ->> 'type'::"text") = ANY (ARRAY['image'::"text", 'video'::"text"]))) THEN "arr"."elem"
                                ELSE "jsonb_build_object"('type', ("arr"."elem" ->> 'type'::"text"), 'url', NULL::"unknown", 'public_id', NULL::"unknown")
                            END ORDER BY "arr"."ord") AS "jsonb_agg"
                       FROM "jsonb_array_elements"(("cp"."media_urls")::"jsonb") WITH ORDINALITY "arr"("elem", "ord"))
                    ELSE NULL::"jsonb"
                END AS "masked_media_urls"
           FROM ( SELECT ((COALESCE("cp"."price_ngn", (0)::numeric) > (0)::numeric) OR (COALESCE("cp"."price_usd", (0)::numeric) > (0)::numeric)) AS "is_paid",
                    ...
                    (("auth"."uid"() IS NOT NULL) AND (EXISTS ( SELECT 1
                          ...
                          WHERE (("pu"."content_type" = 'channel_post'::"text") AND ("pu"."content_id" = ("cp"."id")::"text") AND ("pu"."user_id" = "auth"."uid"()))))) AS "is_unlocked_row") "base") "perm")
...
GRANT ALL ON FUNCTION "public"."expire_channel_subscription"("p_user_id" "uuid", "p_channel_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."expire_channel_subscription"("p_user_id" "uuid", "p_channel_id" "uuid") TO "authenticated";
...
GRANT ALL ON FUNCTION "public"."protect_channel_protected_columns"() TO "anon";
GRANT ALL ON FUNCTION "public"."protect_channel_protected_columns"() TO "authenticated";
...
GRANT ALL ON FUNCTION "public"."subscribe_to_channel"("p_user_id" "uuid", "p_channel_id" "uuid", "p_plan" "text", "p_auto_renew" boolean) TO "anon";
...
GRANT SELECT,REFERENCES,DELETE,TRIGGER,TRUNCATE,MAINTAIN ON TABLE "public"."wallets" TO "authenticated";
